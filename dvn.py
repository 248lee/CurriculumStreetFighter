...
class DVN(OffPolicyAlgorithm):
    ...
    def __init__(
        ...
        self.policy_network_name = policy_network_name
    ...
    def save(self, path) -> None:
        th.save(value_network.state_dict(), path)
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for _ in range(gradient_steps):
            ...
            # Get current V-values estimates
            current_v_values = value_network(replay_data.observations)
    ...
    def collect_rollouts(
        ...
        while should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):
            ...
            if policy_network == None:
                policy_network = PPO.load('trained_models/' + self.policy_network_name)
            policy_network.policy.set_training_mode(False)
            with th.no_grad():
                real_actions, _ = policy_network.predict(self._last_obs)
            # Rescale and perform action
            new_obs, rewards, dones, infos = env.step(real_actions)
