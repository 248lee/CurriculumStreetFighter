...
if __name__ == '__main__':
    ...
    policy_kwargs = dict(
            ...
            net_arch=dict(pi=[], vf=[]),
            features_extractor_class=Stage3CustomFeatureExtractorCNN,
    ...
    for key, value in old_params.items():
        if "cnn_stage" + str(current_stage) in key:
            i += 1
            continue
        print(i, key)
    ...
    for key, value in new_params.items():
        if "cnn_stage" + str(next_stage) + ".0.weight" in key:
        ...
        # elif "cnn_stage2.0.weight" in key:
    ...
    print(model2.get_parameters()['policy']['features_extractor.cnn_stage' + str(current_stage) + '.0.weight'][0][0])
    ...
    print(np.linalg.norm(check_model['features_extractor.cnn_stage' + str(next_stage) + '.0.weight'].cpu().numpy()))
    print(np.linalg.norm(check_model['features_extractor.cnn_stage' + str(current_stage) + '.0.weight'].cpu().numpy()))
